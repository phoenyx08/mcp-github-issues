...
async def make_patch_request(url: str, payload: str) -> dict[str, Any] | None:
    """Make a patch request to the GitHub API with proper error handling."""
    ...
    async with httpx.AsyncClient() as client:
        try:
            ...
            try:
                details = e.response.json()  # GitHub usually returns JSON with "message" + "errors"
            except ValueError:
                details = e.response.text  # fallback to raw text

            return {
                "error": f"Unexpected HTTP error from GitHub: {e}",
